...
app.connect = function(options) {
  ...
  if (options.type == 'component') {
    ...
    // If a password has been provided, prefer the PLAIN SASL mechanism.
    // Otherwise, `node-xmpp` will attempt to use X-OAUTH2 if offered and
    // authentication will fail.  This problem plagues connections to Google
    // Talk, in particular.
    if (options.password) {
      options.preferredSaslMechanism = 'PLAIN';
    }
