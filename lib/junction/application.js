...
app.handle = function(stanza, res, out) {
  ...
  function next(err) {
    ...
    if (!layer) {
      ...
      // If this point is reached, no response (if required) will be sent
      // the the entity.  As a corollary, serviceUnavailable and
      // errorHandler middleware are not in use, against recommendations.
      // We are in the event handler, thus cannot simply throw the error,
      // as this will break the state of EventEmitter. The least we can do
      // is to log the error anyway.
...
function prepareRes(stanza) {
  ...
  if (stanza.is('iq') && (stanza.attr('type') == 'get' || stanza.attr('type') == 'set')) {
    ...
    res = new xml.Element('iq', { id: stanza.attr('id'),
                                  to: stanza.attr('from'),
                                  type: 'result' })
  }
